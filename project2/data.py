...
def state_to_tensor(state: GameState):
    ...
    ghostDistanceWithWalls = get_distance(gridWalls,
                                          pacmanPosition, ghostPosition)
    data.append((ghostDistanceWithWalls / MAX_DIST)
                if ghostDistanceWithWalls is not None else MAX_DIST / MAX_DIST)
    ...
    for x in range(-7, 8):
        for y in range(-7, 8):
            ...
            if 0 <= newX < width and 0 <= newY < height:
                ...
                data.append(1 if ghostPosition[0] == newX
                            and ghostPosition[1] == newY else 0)
